Public Class ContabilidadCRUD
    ...
    Private Sub btnMode_Click(sender As Object, e As EventArgs) Handles btnMode.Click
        ...
        conexion.Consulta("SELECT ID_LibroDiario AS ID, L.Movimiento_LibroDiario AS Movimiento, T.TipoCuenta AS Gestion, L.Monto_TipoCuenta AS Monto, L.Fecha_Movimiento AS Fecha FROM LIBRODIARIO L INNER JOIN TIPOCUENTA T ON T.ID_TipoCuenta = L.ID_TipoCuenta", "LIBRODIARIO", Contabilidad1.dgvVentas)
        conexion.ConsultacmbBox("SELECT ROUND((SELECT SUM(Monto_TipoCuenta) FROM LIBRODIARIO WHERE ID_TipoCuenta = 1) - (SELECT SUM(Monto_TipoCuenta) FROM LIBRODIARIO WHERE ID_TipoCuenta = 2),4) AS [Balance]", "Balance", "Balance", Contabilidad1.cbBalance)
        Me.Close()
    End Sub
End Class